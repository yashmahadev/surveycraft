  ...
  try {
    const forms = await Form.findAll({
      ...
      attributes: [
        ...
        "updatedAt",
        [
          sequelize.fn("COUNT", sequelize.col("responses.id")),
          "totalResponses",
        ],
      ],
      group: [
  ...
  try {
    ...
    if (typeof published !== "boolean") {
      return res.status(400).json({
        message: 'The "published" field must be a boolean (true or false).',
      });
    }
...
exports.updateCustomSlug = async (req, res) => {
  try {
    ...
    if (existingForm) {
      return res
        .status(400)
        .json({ message: "This custom URL is already taken" });
    }
